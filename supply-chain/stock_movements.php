...
$userFilterName = null;
$sql = "SELECT sm.*, p.name product_name, p.sku, u.name user_name, w.name warehouse_name FROM stock_movements sm JOIN products p ON p.id = sm.product_id LEFT JOIN users u ON u.id = sm.created_by LEFT JOIN warehouses w ON w.id = sm.warehouse_id";
$params = [];
if ($userFilter) {
...
<div class="card p-3">
  <div class="table-responsive">
    <table class="table table-hover" id="moveTable">
      <thead><tr><th>Date</th><th>Product</th><th>Warehouse</th><th>Type</th><th class="text-end">Qty</th><th>Reference</th><th>Notes</th><th>By</th></tr></thead>
      <tbody>
      ...
      <?php endforeach; ?>
      <?php if (!$movements): ?><tr><td colspan="8" class="text-muted text-center">No stock movements recorded yet.</td></tr><?php endif; ?>
      </tbody>
    </table>
  </div>
</div>
