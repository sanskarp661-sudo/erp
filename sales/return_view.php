    ...
    try {
        if ($newStatus === 'completed') {
            foreach ($retItems as $it) {
                ...
                stock_move($it['product_id'], $ret['warehouse_id'], $stockQty, 'in', $ret['return_no'], 'Sales return', current_user()['id']);
            }
            ...
            foreach ($retItems as $it) {
                $stockQty = (int)round($it['quantity'] * $it['uom_conversion_factor']);
                stock_move($it['product_id'], $ret['warehouse_id'], -$stockQty, 'out', $ret['return_no'], 'Sales return cancelled', current_user()['id']);
            }
  ...
  <div class="table-responsive">
    <table class="table">
      ...
      <tfoot>
        ...
        <tr><th colspan="4" class="text-end">Credit Note Applied</th><th class="text-end text-success"><?= money($ret['credit_amount']) ?></th></tr>
        <?php endif; ?>
      </tfoot>
    </table>
  </div>
